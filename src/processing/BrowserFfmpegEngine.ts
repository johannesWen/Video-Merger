...
export function createSegmentArgs(inputName: string, outputName: string, item: VideoItem, settings: OutputSettings): string[] {
  const duration = Math.max(0.1, item.metadata?.duration ?? 0.1).toFixed(3);
  const hasAudio = item.metadata?.hasAudio === true;
...
export function createRemuxSegmentsArgs(segmentNames: string[], outputName: string): string[] {
  return [
    "-protocol_whitelist",
    "file,concat",
    "-i",
    `concat:${segmentNames.join("|")}`,
    "-c",
    "copy",
    "-bsf:a",
    "aac_adtstoasc",
